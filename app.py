...
from apex_core import (
    dec_secret, enc_secret, hash_password, verify_password,
    load_db, save_db, get_runtime, _blank_user_settings, _backfill_user,
    send_email_alert, HEARTBEAT_STALE_SEC,
)
...
st.markdown("""
    ...
    @keyframes softPulseAmber {
        0% { box-shadow: 0 0 0 0 rgba(252,213,53,0.35); }
    ...
    @media (max-width: 480px) {
        ...
        .badge-live, .badge-signal, .badge-stale { font-size: 10.5px; padding: 3px 9px; }
...
def add_log(msg):
    core.add_log(db, ACTIVE_USER, msg)
    save_db(db)
...
_hb_age = time.time() - float(rt.get("last_heartbeat", 0) or 0)
_worker_alive = _hb_age < HEARTBEAT_STALE_SEC
st.sidebar.markdown("---")
st.sidebar.markdown("<div class='apex-side-caption'>Worker Service</div>", unsafe_allow_html=True)
if _worker_alive:
    st.sidebar.markdown("<span class='badge-live'>🟢 worker.py connected</span>", unsafe_allow_html=True)
else:
    st.sidebar.markdown("<span class='badge-stale'>🔴 worker.py not detected</span>", unsafe_allow_html=True)
    st.sidebar.caption("Bot ab is browser tab ke bajaye alag `worker.py` process se chalta hai — VPS par usko `python3 worker.py` (tmux/systemd ke saath) chalao taake trading ruke na.")
        ...
        if st.button("🔌 Connect & Verify API", type="primary", use_container_width=True):
            try:
                ...
                user_settings["exchange"] = {"name": ex_choice, "market": market_type,
                                             ...
                                             "demo": demo_chk, "connected": True}
                save_db(db)
                st.success(f"✨ Successfully connected to {ex_choice} ({market_type})! Keys {'encrypted' if core._CRYPTO_OK else 'saved'}.")
            except Exception as e:
                user_settings["exchange"]["connected"] = False
                save_db(db)
                st.error(f"❌ Connection failed: {str(e)}")
...
# ==========================================
    ...
    with mode_tab1:
        st.caption("Sirf jo checkbox tick karoge, bot SIRF wahi condition check karega (AND logic). "
                   "Har indicator ab pichli kuch candles (window) scan karta hai — sirf is exact second wali candle tak limited nahi — "
                   "taake setup milte hi pakda jaye, na ke sirf uss ek second me jab candle close ho rahi ho.")
        manual_cfg = user_settings["strategy"]["manual"]
        st.markdown("<div class='crypto-card'>", unsafe_allow_html=True)
        st.subheader("⏱️ Timeframe (Global)")
        tf_options = ["15m", "1h", "4h", "1d"]
        ...
        st.markdown("<div class='crypto-card'>", unsafe_allow_html=True)
        st.subheader("🌐 Coin Filters (scan universe)")
        flt = user_settings.setdefault("filters", dict(core.DEFAULT_FILTERS))
        flt["universe_min_volume"] = st.number_input(
            "Scan sirf itne 24h Volume (USDT) se upar wale coins par",
            0, 1000000000, value=int(flt.get("universe_min_volume", 1000000)))
        exclude_str = st.text_input(
        ...
        if st.button("💾 Save Manual Strategy", type="primary", use_container_width=True, key="save_manual"):
            ...
            user_settings["strategy"]["mode"] = "manual"
            save_db(db)
            st.success("✅ Manual strategy + filters saved. Worker isi ke hisab se scan karega.")
        ...
        if st.button("📧 Send Test Email", use_container_width=True):
            with st.spinner("Test email bheja ja raha hai..."):
                test_ok = send_email_alert(
                    ...
                    "<html><body style='font-family:Arial;background:#0b0e11;color:#eaecef;padding:20px;'>"
                    "<h2 style='color:#fcd535;'>Test email successful!</h2>"
                    "<p>Agar aapko ye email mila hai, matlab settings bilkul sahi hain.</p>"
                    "</body></html>",
                    email_cfg, log_fn=add_log
                )
            if test_ok:
                st.success("✅ Test email bhej diya gaya! Apna inbox (aur Spam folder) check karo.")
            else:
...
# ==========================================
    ...
    if dash_tab == "📊 Analytics":
        ...
        if not _worker_alive:
            st.markdown("<div class='crypto-card' style='border-left:4px solid #f6465d;'>⚠️ <b>worker.py detect nahi ho raha.</b> "
                        "Bot ab is browser tab me nahi, balke ek alag background service (<code>worker.py</code>) me chalta hai — "
                        "taake phone/laptop band karne ya tab close karne se trading na ruke. Apne server par "
                        "<code>python3 worker.py</code> chalao (tmux ya systemd ke saath, taake wo hamesha chalta rahe), "
                        "phir START BOT dabao.</div>", unsafe_allow_html=True)
        else:
            st.markdown("<div class='crypto-card' style='border-left:4px solid #0ecb81;'>🟢 <b>worker.py</b> chal raha hai aur is account ko monitor kar raha hai — "
                        "tab close, phone lock, ya logout karne se bot NAHI rukega.</div>", unsafe_allow_html=True)
        ...
        c_m2.metric("Successful Wins", successful_wins)
        c_m3.metric("Today's Net PnL", f"${today_pnl:+,.2f}")
        c_m4.metric("Bot Status", "Running" if rt.get("bot_running") else "Stopped")
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        b_c1, b_c2, b_c3 = st.columns([1, 1, 0.6])
        with b_c1:
            if st.button("🚀 START BOT ENGINE", use_container_width=True, type="primary", disabled=bool(rt.get("bot_running"))):
                rt["bot_running"] = True
                add_log("Apex bot engine started (persisted — keeps running via worker.py).")
                st.rerun()
        with b_c2:
            if st.button("🛑 STOP BOT", use_container_width=True, disabled=not bool(rt.get("bot_running"))):
